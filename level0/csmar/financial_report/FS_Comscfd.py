...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "现金流量表(直接法)")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "经营活动产生的现金流量净额")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "购建固定资产、无形资产和其他长期资产支付的现金")]')[0].click()
    time.sleep(SLEEP_TIME)
