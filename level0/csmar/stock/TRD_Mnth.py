...
from utils.csmar_crawler.toggle_time import toggle_time_m
...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "月个股回报率文件")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "月个股总市值")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "考虑现金红利再投资的月个股回报率")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "月收盘价")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "月个股交易股数")]')[0].click()
    time.sleep(SLEEP_TIME)
    
@insure_successful_run(5)
def TRD_Mnth():

    CHINESE_NAME = '月个股回报率文件'
    CODE = 'TRD_Mnth'
