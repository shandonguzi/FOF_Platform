...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "Fama-French因子")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "保存")]')[-1].click()
    time.sleep(SLEEP_TIME)
    
    driver.find_elements(By.XPATH, '//*[contains(text(), "市场风险溢价因子(流通市值加权)")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "市值因子(流通市值加权)")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "账面市值比因子(流通市值加权)")]')[0].click()
    time.sleep(SLEEP_TIME)
