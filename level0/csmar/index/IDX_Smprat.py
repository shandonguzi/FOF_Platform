...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "成分股信息")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "指数成分股权重文件")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "权重(%)")]')[0].click()
    time.sleep(SLEEP_TIME)

@insure_successful_run(5)
def IDX_Smprat():

    CHINESE_NAME = '指数成分股权重文件'
    CODE = 'IDX_Smprat'
