...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "收益分配与拆分")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "基金分配文件")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "场内除息日")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "场外除息日")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "每份分红数")]')[0].click()
    time.sleep(SLEEP_TIME)
