...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "基金拆分信息文件")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "基金份额分拆比例")]')[0].click()
    time.sleep(SLEEP_TIME)
...
def upload_data(CHINESE_NAME, CODE, if_exists):
    ...
    '''
    df = pd.read_csv(f'/data/{CHINESE_NAME}/{CODE}.csv', low_memory=False).fillna(1)
    df.to_sql(CODE, con=level0_csmar, index=False, if_exists=if_exists)

@insure_successful_run(5)
def Fund_Resolution():

    CHINESE_NAME = '基金拆分信息文件'
    CODE = 'Fund_Resolution'
