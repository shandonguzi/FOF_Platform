...
def get_page(driver):
    ...
    driver.find_elements(By.XPATH, '//*[contains(text(), "债券市场系列")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "债券市场交易")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "保存")]')[-1].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "利率及收益率")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "无风险利率文件表")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "无风险利率(%)")]')[0].click()
    time.sleep(SLEEP_TIME)

    driver.find_elements(By.XPATH, '//*[contains(text(), "日度化无风险利率(%)")]')[0].click()
    time.sleep(SLEEP_TIME)
...
def BND_Exchange():
    ...
    if not no_data:
        ...
        if not did_not_update:
            upload_data(CHINESE_NAME, CODE, if_exists='replace')
    driver.close()
