...
def svc_α_ε_generation(months_return, CH3):

    months_return = months_return.dropna()
    temp_dates = months_return.index
    temp_regression = CH3.loc[temp_dates]
    result = LinearRegression().fit(X = temp_regression[['mktrf', 'smb', 'vmg']].values, y = (months_return - temp_regression.rf).values)
    α = result.intercept_
    ε = (result.predict(temp_regression[['mktrf', 'smb', 'vmg']].values) - (months_return - temp_regression.rf).values).std()
    division = α / ε
...
def handle(Fund_Return_m, CH3, Classification):
    ...
    svc_α_ε = regression_m_return_of_risky_fund.rolling(window = 36, min_periods = 36 - MAX_NA_NUM).p_apply(lambda months_return: svc_α_ε_generation(months_return, CH3))
    svc_α_ε.index = svc_α_ε.index + pd.offsets.MonthBegin(0)
    svc_α_ε = svc_α_ε.stack()
    ...
    co_time = np.intersect1d(m_return_of_risky_fund_index, svc_α_ε.index)
    svc_α_ε = svc_α_ε.loc[co_time]

    svc_α_ε.name = 'svc_α_ε'

    return svc_α_ε


def upload(svc_α_ε):
    svc_α_ε.to_sql('svc_α_ε', con=level3_factors, if_exists='replace')


@timeit('level3/svc_α_ε')
def svc_α_ε():
    Fund_Return_m, CH3, Classification = read_sql()
    svc_α_ε = handle(Fund_Return_m, CH3, Classification)
    upload(svc_α_ε)
