...
def read_sql():
    ...
    capm_α = get_sql(level3_factors, 'capm_α', index_cols=['Date', 'Symbol'])

    return Fund_Portfolio_Stock, Classification, capm_α
...
def handle(Fund_Portfolio_Stock, Classification, capm_α):
    ...
    capm_α.index = capm_α.index.set_levels(capm_α.index.levels[0] + pd.offsets.MonthEnd(0), level=0)
    Fund_Portfolio_Stock = pd.merge(Fund_Portfolio_Stock[['Date', 'Symbol', 'Stkcd', 'Proportion', 'v_m_n']], capm_α.reset_index(), on=['Date', 'Symbol'])
    Fund_Portfolio_Stock['δ'] = Fund_Portfolio_Stock.v_m_n * Fund_Portfolio_Stock.capm_α
    ...
    ccp.name = 'connected_companies_portfolio_capm'
    
    return ccp

def upload(ccp):
    ccp.to_sql('connected_companies_portfolio_capm', con=level3_factors, if_exists='replace')

@timeit('level3/connected_companies_portfolio_capm')
def connected_companies_portfolio_capm():
    Fund_Portfolio_Stock, Classification, capm_α = read_sql()
    ccp = handle(Fund_Portfolio_Stock, Classification, capm_α)
    upload(ccp)
