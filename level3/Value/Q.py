...
def handle(data_bs, TRD_Mnth, Stock_last_day):
    ...
    data_bs = data_bs[data_bs.ReportType == 'A'][['Stkcd', 'Date', 'TotalShrhldrEquity', 'TotalAssets', 'DTA', 'DTL']]
    ...
    df_merge['Q'] = (df_merge['MKTValue'] + df_merge['TotalAssets'] - df_merge['TotalShrhldrEquity'] - df_merge['DTA'] + df_merge['DTL']) / df_merge['TotalAssets']
    
    q = df_merge[['Stkcd', 'Date', 'Q']].replace([np.inf, -np.inf], np.nan).dropna()
    q = pd.merge(q, Stock_last_day, on=['Stkcd'])
    q['Date'] = q['Date'] - pd.offsets.MonthBegin(1)
    q = q[(q.Date <= q.LastDay)]
    q = q.drop(['LastDay'], axis=1)

    return q


def upload(q):
    q.to_sql('Q', con=level3_factors, if_exists='replace', index=False)


@timeit('level3/Q')
def Q():
    data_bs, TRD_Mnth, Stock_last_day = read_sql()
    q = handle(data_bs, TRD_Mnth, Stock_last_day)
    upload(q)