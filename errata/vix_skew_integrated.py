...
from settings.database import level3_factors, level4_factor_result, errata_jiayin_robo_advisor
from utils.mysql.get_sql import get_sql
from utils.time_function import timeit
from utils.frequent_dates import next_month_begin, today_s, this_month_begin_s, this_month_begin
...
def get_rebalance_weight(raw_weight, VIX_SKEW):
    ...
    raw_weight_extended.loc[next_month_begin] = raw_weight.iloc[-1].values
    ...
    if raw_weight.index[-1] == next_month_begin:
        risky_weight_daily.loc[today_s] = raw_weight.iloc[-1].filter(like='Risky').values
    ...
    rebalance_weight.loc[next_month_begin, risky_raw_weight.columns] = risky_raw_weight.iloc[-1].values

    # rebalance_weight = rebalance_weight.drop_duplicates().resample('d').bfill()

    advise_on_rebalance = np.abs(changed_part) > .10
    advise_on_entry = VIX_SKEW.vol_and_skew_weight > .55

    rebalance_weight.index = pd.to_datetime(rebalance_weight.index)
    rebalance_weight = rebalance_weight[~rebalance_weight.index.duplicated(keep='first')].sort_index()
    advise_on_entry.index = pd.to_datetime(advise_on_entry.index)
    advise_on_entry = advise_on_entry[~advise_on_entry.index.duplicated(keep='first')].sort_index()
    advise_on_rebalance.index = pd.to_datetime(advise_on_rebalance.index)
    advise_on_rebalance = advise_on_rebalance[~advise_on_rebalance.index.duplicated(keep='first')].sort_index()
...
def upload(total_9, total_10, total_15, total_30, advise_on_rebalance, advise_on_entry):
    ...
    total_9[0].to_sql('total_9_funds_weight_2_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_9[1].to_sql('total_9_funds_weight_3_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_9[2].to_sql('total_9_funds_weight_4_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')

    total_10[0].to_sql('total_10_funds_weight_2_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_10[1].to_sql('total_10_funds_weight_3_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_10[2].to_sql('total_10_funds_weight_4_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')

    total_15[0].to_sql('total_15_funds_weight_2_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_15[1].to_sql('total_15_funds_weight_3_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_15[2].to_sql('total_15_funds_weight_4_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')

    total_30[0].to_sql('total_30_funds_weight_2_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_30[1].to_sql('total_30_funds_weight_3_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    total_30[2].to_sql('total_30_funds_weight_4_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')

    advise_on_rebalance.to_sql('advise_on_rebalance', con=errata_jiayin_robo_advisor, if_exists='replace')
    advise_on_entry.to_sql('advise_on_entry', con=errata_jiayin_robo_advisor, if_exists='replace')
