...
from settings.database import level2_csmar, level4_factor_result
...
def read_sql():
    top_funds = get_sql(level4_factor_result, 'top_funds', index_cols='Date')
...
def upload(top_funds_return):
    top_funds_return.to_sql('top_funds_return', con=level4_factor_result, if_exists='replace')

@timeit('level4/top_funds_return')
